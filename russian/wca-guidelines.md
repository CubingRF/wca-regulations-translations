...
- 2i++++) [УТОЧНЕНИЕ] Считается, что участник использует электронное устройство, только если он задействует функциональность этого устройства.
- 2i+++++) [ПРИМЕР] Не считается использованием, в частности: перемещение камеры, переворачивание телефона, ношение умных часов.
- 2i++++++) [ПРИМЕР] Считается использованием, например: нажатие кнопки на камере, просмотр сообщения на телефоне или умных часах.
